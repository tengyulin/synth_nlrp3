...
for i in mrcPaths:
    ...
    alignOut = os.path.join(pyDir, 'STARs/proj362_%s.star' % state)
    if os.path.exists(alignOut):
        os.remove(alignOut)

    ##########################
    # import angles:
    S2 = pd.read_csv('/home/danlin/synth_nlrp3/4_Projection/tessellations/proj362.txt', header=None, delim_whitespace=True) #projections
